...
class LibraryDb:
    ...
    def missing_metadata_ids(self, limit: int) -> list[str]:
        """Find a bounded batch of videos needing a full metadata lookup.

        Example: `service.backfill(db.missing_metadata_ids(12))`.
        """
        ...
        with self._connect() as conn:
            rows = conn.execute(
                ...
                ORDER BY m.video_id IS NOT NULL DESC, v.discovered_at DESC
                LIMIT ?
                """,
                (limit,),
            ).fetchall()
